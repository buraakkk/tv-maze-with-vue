...
const state = {
  ...
  searchedShows: [],
  tabs: [],
  uniqueGenres: ["Drama", "Action", "Comedy", "Romance", "Horror", "Family"],
};
...
// actions
const actions = {
  showTvShows({ commit, getters }) {
    axios
      .get(`${getters.ENDPOINT}shows`)
      .then((res) => commit(types.FETCH_TV_SHOWS, res.data))
      .catch((err) => console.log(err));
  },
  getTvShowDetails({ commit, getters }, id) {
    axios
      .get(
        `${getters.ENDPOINT}shows/${id}?embed[]=episodes&embed[]=cast&embed[]=crew`
      )
      .then((res) => {
  ...
  searchTvShows({ commit, getters }, search) {
    axios
      .get(`${getters.ENDPOINT}search/shows?q=${search}`)
      .then((res) => {
...
const mutations = {
  [types.FETCH_TV_SHOWS](state, tvshows) {
    ...
    uniqueGenres.forEach((genres) => {
      ...
      tvshows.forEach((element) => {
        if (element.genres.includes(genres))
          genresTvShows[genres].push(element);
      });
  ...
  [types.FETCH_TV_SHOWS_DETAILS](state, tvshowsDetails) {
    ...
    state.seasons = tvshowsDetails._embedded.episodes.reduce(function(r, a) {
      ...
      Object.keys(tvshowsDetails._embedded);
      r[a.season].push(a);
      return r;
    }, Object.create(null));
